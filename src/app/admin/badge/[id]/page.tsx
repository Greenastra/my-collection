...
export default async function EditBadgePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let badge;
  try {
    badge = db.prepare("SELECT * FROM badges WHERE id = ?").get(id);
  } catch (e) {}

  if (!badge) return <div>Значок не найден</div>;
    ...
    <main className="min-h-screen pb-10" style={{ backgroundColor: '#2e0a12', backgroundImage: 'repeating-linear-gradient(90deg, #2e0a12, #2e0a12 15px, #21050a 15px, #21050a 30px)' }}>
      ...
      <header className="w-full max-w-[1200px] mx-auto pt-4 relative z-10">
        <div className="relative w-full h-[180px] md:h-[220px] border-b-4 border-[#cc0000] bg-[#21050a] shadow-lg">
          ...
          <div className="absolute top-0 right-0 bg-red-600 text-white px-3 py-1 font-bold uppercase shadow-md">
            Редактирование
          </div>
        </div>
        <nav className="bg-[#1a0505] text-white py-2 px-4 flex justify-between items-center border-b border-[#333] shadow-md">
          ...
          <Link href={`/admin?tab=catalog&path=${encodeURIComponent(badge.folder_path)}`} className="bg-gray-700 px-4 py-1 rounded hover:bg-gray-600 font-bold text-sm">
            Отмена / Назад
          </Link>
        </nav>
      </header>
          ...
          <form action={saveBadge.bind(null, badge.id)}>
            <div className="flex items-center justify-between border-b-2 border-[#eee] mb-6 pb-2">
              <div className="w-full mr-4">
                ...
                <input type="text" name="name" defaultValue={badge.name} className="w-full text-3xl font-serif font-bold text-[#800000] border-b-2 border-gray-300 focus:border-[#800000] outline-none" />
              </div>
            </div>
            ...
            <div className="flex flex-col xl:flex-row gap-8">
              ...
              <div className="w-full xl:w-1/2 space-y-6 text-lg">
                <div className="p-6 bg-gray-50 border-l-4 border-[#cc0000] shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input type="text" value={badge.folder_path} readOnly className="w-full bg-gray-200 border border-gray-300 rounded p-2 text-gray-600 text-sm font-mono" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 uppercase">Год</label>
                        <input type="text" name="year" defaultValue={badge.year} className="w-full border-2 border-gray-300 rounded p-2 focus:border-[#800000] outline-none" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-gray-700 uppercase">Материал</label>
                        <input type="text" name="material" defaultValue={badge.material} className="w-full border-2 border-gray-300 rounded p-2 focus:border-[#800000] outline-none" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end pt-4 border-t border-gray-200">
                  <button className="bg-[#800000] text-white px-8 py-3 rounded font-bold text-lg hover:bg-[#600000] shadow-lg transform hover:scale-105 transition">
                    СОХРАНИТЬ ИЗМЕНЕНИЯ
                  </button>
                </div>
              </div>
