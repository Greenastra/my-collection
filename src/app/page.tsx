    ...
    <main className="min-h-screen pb-10" style={{ backgroundColor: '#2e0a12', backgroundImage: 'repeating-linear-gradient(90deg, #2e0a12, #2e0a12 15px, #21050a 15px, #21050a 30px)' }}>
      <header className="w-full max-w-[1200px] mx-auto pt-4 relative z-10">
        ...
        <nav className="bg-[#1a0505] text-white py-2 px-4 flex flex-wrap gap-6 text-xl font-bold border-b border-[#333] shadow-md justify-center md:justify-start">
          ...
          <Link href={`/catalog/${encodeURIComponent('Региональный комсомол')}`} className="hover:text-[#ffcc00] hover:underline transition">Региональный комсомол</Link>
          <Link href={`/catalog/${encodeURIComponent('ССО')}`} className="hover:text-[#ffcc00] hover:underline transition">ССО</Link>
          <Link href={`/catalog/${encodeURIComponent('ВЛКСМ')}`} className="hover:text-[#ffcc00] hover:underline transition">ВЛКСМ</Link>
          <Link href={`/catalog/${encodeURIComponent('Разное')}`} className="hover:text-[#ffcc00] hover:underline transition">Разное</Link>
      ...
      <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row mt-6 gap-4 px-2">
        <aside className="w-full md:w-[320px] flex flex-col gap-4 flex-shrink-0">
           <div className="bg-[#2a0a0a] p-3 border-2 border-[#3d1414] shadow-2xl relative h-[400px]">
        ...
        <section className="flex-1 bg-white rounded p-8 shadow-2xl relative">
          ...
          <div className="flex flex-col lg:flex-row gap-6 relative z-10">
            ...
            <div className="w-full lg:w-[280px] flex-shrink-0">
              <div className="relative w-full aspect-[3/4] rounded shadow-md border border-gray-300 overflow-hidden">
                 <Image src="/header.jpg" alt="Владелец коллекции" fill className="object-cover" unoptimized />
              </div>
            </div>

            {/* Текст */}
            <div className="flex-1 text-lg text-gray-800 font-serif leading-relaxed space-y-4">
              <p>
              ...
              <p className="font-bold text-[#800000] mt-4">
                Все знаки и значки посвященные комсомолу можно условно классифицировать на следующие группы:
              </p>
              <ul className="list-decimal list-inside space-y-1 ml-2 text-base bg-gray-50 p-3 rounded border border-gray-200">
              ...
              <div className="mt-6 p-4 bg-[#fff8f0] border-l-4 border-[#cc0000] italic text-gray-700 font-bold text-xl text-right">
                С Уважением, Влад
              </div>
            </div>
          </div>
